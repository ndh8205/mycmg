%% main_pos_mppi_mex_v3.m - MPPI v3 Position Control Test (SMC Reference)
% Waypoint following with SMC reference rollouts
clear; clc; close all;
...
%% MPPI Parameters with SMC gains
...
mppi_params.w_smooth = 11;
mppi_params.crash_cost = 10000;
mppi_params.crash_angle = deg2rad(80);

% SMC rollout parameters
mppi_params.K_smc = 32;
mppi_params.sigma_smc = 0.2;

% Position SMC gains (from main_pos_tuning_hexa_smc.m)
mppi_params.smc_pos_a = [10; 10; 10];
mppi_params.smc_pos_l1 = [0.8; 0.8; 0.8];
mppi_params.smc_pos_l2 = [0.3; 0.3; 0.55];
mppi_params.smc_pos_r = 0.98;

% Attitude SMC gains (from main_att_tuning_hexa_smc.m)
mppi_params.smc_att_a = 15;
mppi_params.smc_att_b = 15;
mppi_params.smc_att_l1 = 5.4;
mppi_params.smc_att_l2 = 5.4;
mppi_params.smc_att_r = 0.95;
...
%% Simulation loop
fprintf('Running MPPI v3 (SMC) position control (dist: %s)...\n', dist_preset);
tic_total = tic;
...
for k = 1:N_sim-1
    ...
    if mod(k-1, ctrl_decimation) == 0
        ...
        MPPI_SMC(k) = mppi_state.stats.best_is_smc;
    end
...
%% Plot results
figure('Position', [100 100 1400 900], 'Name', 'MPPI v3 (SMC) Position Control');
...
% SMC selection rate
subplot(3,4,11);
smc_idx = find(MPPI_SMC > -1);
smc_rate = movmean(MPPI_SMC(smc_idx), 10) * 100;
plot(t(smc_idx), smc_rate, 'b', 'LineWidth', 1);
xlabel('Time [s]'); ylabel('SMC Best [%]');
title('SMC Reference Selection'); grid on;
ylim([0 100]);
...
sgtitle(sprintf('MPPI v3 (SMC Reference) Position Control (Dist: %s)', dist_preset));

%% Print summary
fprintf('\n=== MPPI v3 (SMC) Position Control Summary ===\n');
fprintf('Preset: %s\n', dist_preset);
...
filename = sprintf('pos_mppi_v3_smc_%s.csv', dist_preset);
